...
import { Loader2, BarChart3, AlertTriangle } from 'lucide-vue-next'
...
const sortedOems = computed(() => {
  if (!analytics.value?.by_oem) return []
...
const maxPatternCount = computed(() => {
  if (!analytics.value?.by_pattern) return 1
  ...
  <BasicPage title="Recipe Analytics" description="Recipe coverage, pattern distribution, and gap analysis across all OEMs.">
    ...
    <div v-else-if="analytics" class="space-y-8">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <UiCard class="p-4">
          <p class="text-xs text-muted-foreground">Brand Recipes</p>
          <p class="text-2xl font-bold">{{ analytics.total_brand }}</p>
        </UiCard>
        <UiCard class="p-4">
          <p class="text-xs text-muted-foreground">Default Recipes</p>
          <p class="text-2xl font-bold">{{ analytics.total_default }}</p>
        </UiCard>
        <UiCard class="p-4">
          <p class="text-xs text-muted-foreground">OEMs with Recipes</p>
          <p class="text-2xl font-bold">{{ Object.keys(analytics.by_oem).length }}</p>
        </UiCard>
        <UiCard class="p-4">
          <p class="text-xs text-muted-foreground">OEMs with Gaps</p>
      ...
      <UiCard class="p-6 overflow-x-auto">
        <h2 class="text-lg font-semibold mb-4">Coverage Matrix</h2>
        <table class="w-full text-xs">
          <thead>
            <tr class="border-b">
              <th class="px-2 py-1.5 text-left font-medium">OEM</th>
              ...
              <th class="px-2 py-1.5 text-center font-medium">Total</th>
            </tr>
          </thead>
          <tbody class="divide-y">
            <tr v-for="oem in sortedOems" :key="oem.oemId">
              <td class="px-2 py-1.5 font-medium">{{ oemLabel(oem.oemId) }}</td>
              <td
      ...
      <UiCard v-if="analytics.gaps.length" class="p-6">
        <h2 class="text-lg font-semibold mb-4 text-amber-600">Coverage Gaps</h2>
        <div class="space-y-2">
