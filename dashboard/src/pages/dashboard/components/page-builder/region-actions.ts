...
export function getRegionActions(region: CloneRegion): RegionAction[] {
  ...
  if (hasKind(region, 'text'))
    out.push({ id: 'edit-text', label: 'Edit text', group: 'content' })
  ...
  if (hasKind(region, 'link'))
    out.push({ id: 'edit-link', label: 'Edit link / button…', group: 'content' })
