...
describe('pageBuilderCanvas preview mode', () => {
  ...
  it('renders a compact clone selection toolbar for quick text styling', () => {
    ...
    expect(source).toContain('emit(\'regionAction\', { action: \'duplicate\', regionId: region.id, html: region.html })')
    expect(source).toContain('emit(\'regionAction\', { action: \'convert\', regionId: region.id, html: region.html })')
    expect(source).toContain('emit(\'regionAction\', { action: \'delete\', regionId: region.id, html: region.html })')
