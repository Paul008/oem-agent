...
const variant = computed(() => {
  ...
  if (t === 'embed')
    return 'video' // embed uses same rendering as video
  if (t === 'image')
    return 'image-showcase' // single image → showcase
  return t
})
...
const embedUrl = computed(() => {
  ...
  if (yt)
    return `https://www.youtube.com/embed/${yt[1]}?rel=0${props.section.autoplay ? '&autoplay=1&mute=1' : ''}`
  const vm = url.match(/vimeo\.com\/(?:video\/)?(\d+)/)
  if (vm)
    return `https://player.vimeo.com/video/${vm[1]}?${props.section.autoplay ? 'autoplay=1&muted=1' : ''}`
  if (props.section.embed_url)
    return props.section.embed_url // generic iframe
  return null
})
...
const showcaseImages = computed(() => {
  if (props.section.images?.length)
    return props.section.images
  if (props.section.desktop_image_url)
    return [{ url: props.section.desktop_image_url, alt: props.section.alt, caption: props.section.caption }]
  return []
})
</script>
...
<template>
    ...
    <Teleport to="body">
      <div v-if="lightboxIndex !== null && section.images?.[lightboxIndex]" class="fixed inset-0 z-[100] bg-black/80 flex items-center justify-center" @click.self="closeLightbox">
        <div class="relative max-w-4xl w-full mx-4 bg-card rounded-xl overflow-hidden shadow-2xl max-h-[90vh] flex flex-col">
          <button class="absolute top-3 right-3 z-10 p-1.5 bg-black/50 hover:bg-black/70 rounded-full text-white transition-colors" @click="closeLightbox">
            <X class="size-5" />
          </button>
          <div class="relative bg-muted">
            ...
            <button v-if="section.images.length > 1" class="absolute left-3 top-1/2 -translate-y-1/2 p-2 bg-black/50 hover:bg-black/70 rounded-full text-white" @click.stop="prevImage(section.images.length)">
              <ChevronLeft class="size-5" />
            </button>
            <button v-if="section.images.length > 1" class="absolute right-3 top-1/2 -translate-y-1/2 p-2 bg-black/50 hover:bg-black/70 rounded-full text-white" @click.stop="nextImage(section.images.length)">
          ...
          <div v-if="section.images[lightboxIndex].caption || section.images[lightboxIndex].description" class="px-6 py-4 border-t">
            ...
            <p v-if="section.images[lightboxIndex].description" class="text-sm text-muted-foreground mt-1.5">
  ...
  <div v-else-if="variant === 'video'" :class="videoLayout === 'full-width' ? '' : 'py-6 px-8'">
    <h3 v-if="section.title" class="text-xl font-bold mb-4" :class="{ 'px-8 pt-6': videoLayout === 'full-width' }">
      {{ section.title }}
    </h3>
  ...
  <!-- Fallback -->
  <div v-else class="px-8 py-10 text-sm text-muted-foreground">
    No media content
  </div>
</template>
