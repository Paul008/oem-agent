...
<template>
  <div class="px-8 py-10">
    <h3 class="text-xl font-bold mb-2 cursor-text outline-none" :style="{ opacity: section.title ? 1 : 0.4 }" @dblclick="startEditing('title', titleEdit, $event)" @blur="titleEdit.stopEdit()" @keydown="titleEdit.onKeydown" @paste="titleEdit.onPaste">
      {{ section.title || 'Double-click to add title' }}
    </h3>
    <p class="text-sm text-muted-foreground mb-6 cursor-text outline-none" :style="{ opacity: section.sub_heading ? 1 : 0.4 }" @dblclick="startEditing('sub_heading', subEdit, $event)" @blur="subEdit.stopEdit()" @keydown="subEdit.onKeydown" @paste="subEdit.onPaste">
    ...
    <div v-else class="border-2 border-dashed border-muted-foreground/30 rounded-lg p-8 text-center bg-muted/20 h-[400px] flex items-center justify-center">
      <p class="text-sm text-muted-foreground">
        Paste a Google Maps embed URL to display the map
      </p>
    </div>
  </div>
</template>
