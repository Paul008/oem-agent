...
import { addWebhook, deleteWebhook, fetchWebhooks } from '@/lib/worker-api'
...
async function handleAdd() {
  if (!newUrl.value || !newEvents.value.size)
    return
  adding.value = true
  try {
...
function toggleEvent(event: string) {
  if (newEvents.value.has(event))
    newEvents.value.delete(event)
        ...
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y">
            <tr v-for="wh in webhooks" :key="wh.id">
              <td class="px-4 py-2.5 font-mono text-xs truncate max-w-xs">
                {{ wh.url }}
              </td>
