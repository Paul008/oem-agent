...
export function buildCaptureInjection(): { earlyStub: string, lateInjection: string } {
  ...
  // Inject shared Tailwind rules from the extracted module
  ...
  function tailwindHtml(el) {
    ...
    function convert(src, cln) {
      ...
      if (src.className) {
        var mapped = R.mapClasses(src.className);
      ...
      // Apply per-side border rules (width, style, color) via dedicated borderTw reader (radius is handled in cssTw)
