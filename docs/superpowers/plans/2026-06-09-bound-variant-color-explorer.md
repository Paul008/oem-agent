...
- [x] Add `variant-color-explorer` to `PageSectionType`.
- [x] Add defaults with `oem_id`, `model_slug`, `eyebrow`, `heading`, `cta_text`, `cta_url`, `data_source`, and fallback `variants`.
- [x] Register `SectionVariantColorExplorer.vue` in canvas and display maps.
- [x] Add a section type label and icon.
- [x] Run dashboard tests and confirm registry coverage still passes.
...
- [x] Add enough type fields on `VariantColor` for explorer rendering (`hero_image_url`, `swatch_url`, `gallery_urls`, `hex_code`, `price_delta`, `sort_order` if not already typed).
- [x] Reuse `fetchProductsForModel` and `fetchVariantColors` rather than adding a new worker endpoint.
- [x] Keep fallback behavior client-side: no products or no colours should still render manual section data.
...
- [x] Load products for `section.oem_id || props.oemId` and `section.model_slug || props.modelSlug`.
- [x] Load colours for the loaded product IDs.
- [x] Normalize products into tabs with title, description, price label, key features, specs fallback, CTA, and colour list.
- [x] Render Mitsubishi-style layout: centred eyebrow/heading, horizontal grade tabs, left detail panel, image stage, colour swatches, key-features disclosure, CTA.
- [x] Implement mobile/tablet layout with horizontal tabs, image before swatches, and no text/image overlap.
- [x] Add empty/fallback state using manual section variants.
...
- [x] Add `variant-color-explorer` controls for data source, OEM override, model slug override, eyebrow, heading, CTA text, and CTA URL.
- [x] Keep controls compact and avoid forcing manual editing of database-derived variants in this first slice.
...
- [x] Confirm TypeScript passes.
- [x] Commit with a focused message.
- [x] Push `main`.
- [x] Deploy with `pnpm exec wrangler pages deploy dashboard/dist --project-name oem-dashboard --branch main`.
