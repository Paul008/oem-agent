...
## Runtime-Safe Body Patterns

This HTML is injected as static body markup. Do NOT emit Alpine.js, Vue, Svelte, React, inline
scripts, or framework directives. Never use \`x-data\`, \`x-show\`, \`x-for\`, \`x-text\`,
\`x-collapse\`, \`@click\`, \`:src\`, \`:class\`, or \`:style\`. Interactive components are handled
later by structured Vue sections; raw generated HTML must remain readable without JavaScript.

### Colour Range
- Render a static colour section with a prominent vehicle image from the first available
  \`hero_image_url\`, then a visible swatch grid using real \`swatch_url\` / \`hero_image_url\` values.
- Show color names and pricing badges as normal text.
- If \`gallery_urls\` are available, render them as a horizontal \`overflow-x-auto snap-x\` image row.
- Do not emit \`vehicle360()\` or hidden frame stacks; the Vue \`Vehicle360Viewer\` handles 360°
  behavior only after content is represented as a structured color-picker section.

### Specifications
- Use native \`details\` / \`summary\` elements for expandable spec groups, or a visible responsive
  grid/table when there are only a few specs.
- Populate rows from real \`specs_json\`.
- Keep at least the first group open with \`open\` so content is visible without any script.

### Galleries, Slideshows, and Media Rows
- Use static image grids or horizontal \`overflow-x-auto snap-x snap-mandatory\` rows.
- Wrap large images in normal \`<a href="IMAGE_URL" target="_blank" rel="noopener">\` links when a
  larger view is useful.
- Do not build lightboxes, timers, hidden slides, or script-driven controls in raw HTML.

### Tabs / Grouped Content
- Do not hide content behind tab state. Render each group as a visible section with a heading.
- If a tab-like overview is needed, add an anchor-link row at the top and visible anchored groups
  below it.
- Include actual images from the Available Images list and match the visual extraction data.
...
The returned \`content.rendered\` HTML is static body markup. Do NOT use Alpine.js, Vue, Svelte,
React, inline scripts, or framework directives. Never emit \`x-data\`, \`x-show\`, \`x-for\`,
\`x-text\`, \`x-collapse\`, \`@click\`, \`:src\`, \`:class\`, or \`:style\`.

- **Color range**: render a static hero image plus visible swatches from real color data. Use
  horizontal \`overflow-x-auto snap-x\` rows for any additional gallery images.
- **Specs**: use native \`details\` / \`summary\` groups or a visible responsive grid/table. Keep the
  first group open with the native \`open\` attribute.
- **Gallery/slideshow**: use visible grids or scroll-snap rows with normal image links. Do not create
  hidden slide stacks, lightboxes, timers, or script-driven controls in raw HTML.
- **Tabbed content**: render visible grouped sections with headings, or an anchor-link row that jumps
  to visible anchored content below. Do not hide panels behind runtime state.
